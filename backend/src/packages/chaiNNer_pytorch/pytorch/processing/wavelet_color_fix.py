...
def wavelet_blur(image: torch.Tensor, radius: int) -> torch.Tensor:
    kernel_vals = [[0.0625, 0.125, 0.0625], [0.125, 0.25, 0.125], [0.0625, 0.125, 0.0625]]
...
def wavelet_reconstruction(content_feat: torch.Tensor, style_feat: torch.Tensor, levels: int) -> torch.Tensor:
...
@processing_group.register(
    ...
    outputs=[
        ImageOutput().with_never_reason("Returns the color-fixed image.")
    ],
    node_context=True,
)
def wavelet_color_fix_node(context, target_img: np.ndarray, source_img: np.ndarray, levels: int) -> np.ndarray:
    target_h, target_w, _ = get_h_w_c(target_img)
    
    #resize source image to match target image
    source_img_resized = resize(source_img, (target_w, target_h), filter=ResizeFilter.BOX)
    ...
    #wavelet color fix
    result_tensor = wavelet_reconstruction(target_tensor, source_tensor_resized, levels=levels)

    #convert back to numpy array
    result_img = tensor2np(result_tensor.detach().cpu(), change_range=False, imtype=np.float32)
