...
def one_channel_nearest_uniform_color(value: Union[np.ndarray, float], num_colors: int) -> float:
    return np.floor(value * (num_colors - 1) + 0.5) / (num_colors - 1)


def one_channel_batch_nearest_uniform_color(image: np.ndarray, num_colors: int) -> np.ndarray:
...
def batch_nearest_uniform_color(image: np.ndarray, num_colors: int) -> np.ndarray:
    return apply_to_all_channels(
        one_channel_batch_nearest_uniform_color,
        image,
