...
def one_channel_uniform_quantize(image: np.ndarray, num_colors: int) -> np.ndarray:
    out_image = uniform_quantize_image(dtype_to_float(image), num_colors=num_colors)
...
def uniform_quantize(image: np.ndarray, num_colors: int) -> np.ndarray:
    return apply_to_all_channels(
        one_channel_uniform_quantize, image, num_colors=num_colors
    )
