...
def red_weighted_euclidean_color_distance(
    pixel: np.ndarray, color: np.ndarray
) -> float:
...
def non_red_weighted_euclidean_color_distance(
    pixel: np.ndarray, color: np.ndarray
) -> float:
    # https://www.compuphase.com/cmetric.htm
    # weights that work well when red < 0.5
...
def adaptive_weighted_euclidean_color_distance(
    pixel: np.ndarray, color: np.ndarray
) -> float:
    # https://www.compuphase.com/cmetric.htm
    ...
    return (
        np.dot(np.power(pixel - color, 2).mean(), [2 + mean_red, 4, 3 - mean_red]).sum()
        / 9
    )
...
def _batch_prepare_inputs(
    image: np.ndarray, color: np.ndarray, expect_rgb=False
    ...
    if expect_rgb and color.shape != 3:
        raise RuntimeError(
            "weighted euclidean color distance assumes a certain colorspace (probably RGB)"
        )

    return dtype_to_float(image), dtype_to_float(color)


def _batch_weighted_euclidean_color_distance(
    image: np.ndarray, color: np.ndarray, weights: np.ndarray
) -> np.ndarray:
...
def batch_red_weighted_euclidean_color_distance(
...
def batch_non_red_weighted_euclidean_color_distance(
    image: np.ndarray, color: np.ndarray
) -> np.ndarray:
    # https://www.compuphase.com/cmetric.htm
    # weights that work well when red < 0.5
...
def batch_adaptive_weighted_euclidean_color_distance(
    image: np.ndarray, color: np.ndarray
) -> np.ndarray:
    # https://www.compuphase.com/cmetric.htm
    # Interpolate the "red" and "non-red" weights above based on mean red value
    ...
    weights = np.stack(
        [2 + mean_red, np.full(image.shape[:2], 4), 3 - mean_red], axis=2
    )
    max_distance = 2 + 4 + 3
