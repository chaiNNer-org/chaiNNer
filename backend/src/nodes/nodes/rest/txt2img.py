...
from ...impl.rest import decode_base64_image, SamplerName, STABLE_DIFFUSION_TEXT2IMG_URL, post_async
...
class Txt2Img(AsyncNodeBase):
    def __init__(self):
        super().__init__()
        self.description = "Generate an image using an external Stable Diffusion service"
        self.inputs = [
            ...
            NumberInput("CFG Scale", minimum=1, default=7, maximum=20, precision=4, controls_step=0.1),
    ...
    async def run_async(self, prompt: str, negative_prompt: str, seed: int, steps: int, sampler_name: SamplerName,
                        cfg_scale: float, width: int, height: int, sd_model_checkpoint: str) -> np.ndarray:
        request_data = {
            "prompt": prompt,
        ...
        if sd_model_checkpoint != "":
            request_data['override_settings']['sd_model_checkpoint'] = sd_model_checkpoint
