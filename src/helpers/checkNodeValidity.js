...
const checkNodeValidity = ({
  ...
  const isMissingInputs = nonOptionalInputs.length + enteredOptionalInputs.length > Object.keys(inputData).length + filteredEdges.length;
  if (isMissingInputs || emptyInputs.length > 0) {
    ...
    const missingInputs = nonOptionalInputs.filter(
      (input, i) => !edgeTargetIndexes.includes(String(i))
       && (!Object.keys(inputData).includes(String(i))
         || emptyInputs.includes(String(i))),
    );
